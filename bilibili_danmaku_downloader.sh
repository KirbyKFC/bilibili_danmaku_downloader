...
get_danmaku () {
	...
	cid=$( echo "$cidData" | grep -E -A1 '"cid"' | grep -E "[0-9]+" | head -1)
	titleData=$( curl -s "https://api.bilibili.com/x/web-interface/view?bvid=$target" | sed 's/\":/\"\n/g;s/,/\n/g' ) 
	#titleData=$( curl -s "https://api.bilibili.com/x/web-interface/view?cid=$cid&bvid=$target" | sed "s/:/\\n/g;s/,/\\n/g" ) 
	title=$( echo -e "$titleData" | grep -E -A1 '"title"' | sed 's/[\"\\\/\<\>\*\?\:\|]//g')
	if test -z "$title"
	then
		title="000000$cid"
	fi
	curl -s -k -L --compressed -o "$cid.xml" "http://comment.bilibili.com/$cid.xml"
	echo "${title:6}.ass"
	danmaku2ass -f Bilibili -o "${title:6}.ass" -s "$screen_size" -fn "$font" -fs $font_size -a $alpha -dm $duration_marquee -ds $duration_still -fl "$danmaku_filter" "$cid.xml"
	rm "$cid.xml"
